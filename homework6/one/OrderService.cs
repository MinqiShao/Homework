    ...
    public class OrderService
    {
        ...
        public void changeDetails(int id,string gname,int num)  //修改明细
        {
            try
            {
                var query = list.Where(s => s.ID == id);
                Order order = query.FirstOrDefault();
                OrderDetails detail = order.Details.Where(s => s.Goods.GoodsName == gname).FirstOrDefault();
                detail.Number = num;
            }
            catch (Exception e)
            {
        ...
        public void Import()   //反序列化，从XML载入订单
        {
            XmlSerializer xmlSerializer = new XmlSerializer(typeof(Order[]));
            using(FileStream fs=new FileStream("order.xml", FileMode.Open))
            {
                Order[] _list = (Order[])xmlSerializer.Deserialize(fs);
                Console.WriteLine("由XML反序列化得到：");
                Array.ForEach(_list, p => Console.WriteLine(p));
            }
        }
    }
}
